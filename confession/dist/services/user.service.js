"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.loginWithEmailAndPassword = exports.createUser = void 0;
const http_status_1 = __importDefault(require("http-status"));
const user_model_1 = __importDefault(require("../models/user.model"));
const apiError_1 = __importDefault(require("../utils/apiError"));
/**
 * Create a new user
 * @param {UserBody} userBody - The user's data
 * @returns {Promise<UserDocument>}
 */
const createUser = (userBody) => __awaiter(void 0, void 0, void 0, function* () {
    const isEmailTaken = yield user_model_1.default.isEmailTaken(userBody.email);
    if (isEmailTaken) {
        throw new apiError_1.default(http_status_1.default.BAD_REQUEST, "Email is already taken");
    }
    return user_model_1.default.create(userBody);
});
exports.createUser = createUser;
const loginWithEmailAndPassword = (email, password) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = yield user_model_1.default.findOne({ email });
        if (!user) {
            throw new Error("Email or password is incorrect");
        }
        const isPasswordValid = yield user.isPasswordMatch(password);
        if (!isPasswordValid) {
            throw new Error("Email or password is incorrect");
        }
        return user;
    }
    catch (error) {
        throw new apiError_1.default(http_status_1.default.UNAUTHORIZED, "Email or password is incorrect");
    }
});
exports.loginWithEmailAndPassword = loginWithEmailAndPassword;
//# sourceMappingURL=user.service.js.map